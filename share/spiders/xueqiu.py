...
class XueqiuSpider(BaseSpider):
    ...
    def _check_res(res):
        if res and 'error_code' in res and res['error_code'] == 0:
            return res['data']
        # else
        raise Exception(res['error_description'] if 'error_description' in res.keys() else '')
    ...
    def url_generator(self, btypes=['zxb', 'sza', 'sha'], size=30):
        ...
        for btype in btypes:
            ...
            try:
                res = XueqiuSpider._check_res(json.loads(self.get(url).content.decode('utf-8')))
    ...
    def parse(self, response):
        try:
            items = XueqiuSpider._check_res(json.loads(response.content.decode('utf-8')))
        except Exception as e:
            raise Exception(e)
